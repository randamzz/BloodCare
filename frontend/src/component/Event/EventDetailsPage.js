...
const EventDetailsPage = () => {
    ...
    const fetchEventDetails = async () => {
      try {
        if (eventId) {
          ...
          const response = await axios.get(`http://localhost:8000/Event/events/${eventId}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await axios.post(`http://localhost:8000/Event/api/events/${eventId}/submit_email/`, {
        email: email
      });
      setSuccessMessage('Email submitted successfully. Check your inbox for the ticket!');
      setEmail('');
    } catch (error) {
    ...
    <div className="event-details-container">
      {eventDetails ? (
        ...
        <p>__________________________________________________________</p>
          ...
          <p>Date: {new Date(eventDetails.date_and_hour).toLocaleDateString()}</p>
          <p>Time: {new Date(eventDetails.date_and_hour).toLocaleTimeString()}</p>
        ...
        <form onSubmit={handleSubmit}>
          <input type="email" value={email} placeholder="Email Address" onChange={handleEmailChange} required />
